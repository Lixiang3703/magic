...
@interface YYGalleryPhotoView (Private)
...
@implementation YYGalleryPhotoView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    [_imageView setImage:[UIImage imageNamed:@"img_picture_bg@2x.png"]];
	_imageView.contentMode = UIViewContentModeScaleAspectFit;
	[self addSubview:_imageView];
	
	// create an activity inidicator
	_activity = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
    [_activity setCenter:CGPointMake(self.frame.size.width/2, self.frame.size.height/2)];
...
- (id)initWithFrame:(CGRect)frame target:(id)target action:(SEL)action
{
    ...
    [self.layer setShadowColor:[[UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:1] CGColor]];
...
- (void)startTapTimer {
	_tapTimer = [[NSTimer alloc] initWithFireDate:[NSDate dateWithTimeIntervalSinceNow:0.2] interval:0.5 target:self selector:@selector(handleTap) userInfo:nil repeats:NO];
