//
//  KKModifyCompanyViewController.m
//  Magic
//
...
@interface KKModifyCompanyViewController ()
...
@implementation KKModifyCompanyViewController
...
- (void)setupRawItemFromUI {
    ...
    self.modifyPersonItem.company = self.nameModifyTextField.text;
}
...
- (void)reloadDataSourceWithRecordFromUI:(BOOL)recordFromUi {
    ...
    self.nameModifyCellItem.titleName = self.modifyPersonItem.company;
...
- (void)rightBarButtonItemClick:(id)sender {
    ...
    if (![self.nameModifyTextField.text hasContent]) {
        [UIAlertView postAlertWithMessage:_(@"请输入公司名称")];
        return;
    }
